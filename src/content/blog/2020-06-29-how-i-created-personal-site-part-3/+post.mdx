...
cover_image: ./camera-scenery.jpg
...
<mark>A Note though</mark>: If you're a company or someone who wants to save
time and money, go with Cloudinary or some other similar solution. Cloudinary
optimized images are much smaller too.
...
<mark>Note</mark>: The statement to convert to `webp` should come before
optimizing the `jpeg`s themselves, or the quality of `webp` images will be quite
aweful.
